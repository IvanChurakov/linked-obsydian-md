[Link to the course](https://learn.microsoft.com/en-us/training/modules/explore-microsoft-identity-platform/)
...
In most common cases, Conditional Access doesn't change an app's behaviour or require any changes from the developer. Only in certain cases when an app indirectly or silently requests a token for a service does an app require code changes to handle Conditional Access challenges. It may be as simple as performing an interactive sign-in request.
...
- [ ] Apps performing the Integrated Windows authentication flowz
